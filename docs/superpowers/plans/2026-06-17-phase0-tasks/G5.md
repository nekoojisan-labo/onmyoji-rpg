...
> 注（2026-06-17 実装時修正・strict tsconfig 対応）: ①G5.3 `stats.ts` の `import type { Element } from '../types';` は未使用なので削除（`noUnusedLocals` で TS6133）。②G5.5 `leveling.ts` の `GROWTH[0]` / `GROWTH[i]` は `noUncheckedIndexedAccess` で `GrowthRow | undefined` になるため `GROWTH[0]!` / `const row = GROWTH[i]!;` の非null断定に変更（GROWTHは非空固定テーブルで添字は常に有効）。挙動は不変・テストは全PASS。実装済みファイルが正。
